...
namespace Charity.Infra.Repository
{
	public class TestimonialRepository : ITestimonialRepository
	{
		...
		{
			IEnumerable<Testimonials> result = DbContext.Connection.Query<Testimonials>("Testimonial_Package.GetAllTestimonials", commandType: CommandType.StoredProcedure);
		...
		public CharityTestimonial GetTestimonialById(int id)
		{
			...
			IEnumerable<CharityTestimonial> result = DbContext.Connection.Query<CharityTestimonial>("Testimonial_Package.GetTestimonialById", p, commandType: CommandType.StoredProcedure);
			return result.FirstOrDefault();
		}
        public List<Testimonials> GetTestimonialByUserId(int user_id)
        {
            var p = new DynamicParameters();
            p.Add("t_user_id", user_id, dbType: DbType.Int32, direction: ParameterDirection.Input);

            IEnumerable<Testimonials> result = DbContext.Connection.Query<Testimonials>("Testimonial_Package.GetTestimonialByUserId", p, commandType: CommandType.StoredProcedure);
            return result.ToList();
        }
